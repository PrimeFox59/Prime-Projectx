...
def initialize_defaults():
    """Populate database with default data if empty (from CV/Portfolio)."""
    try:
        def _slug(text: str) -> str:
            return ''.join(ch.lower() if ch.isalnum() else '-' for ch in text).strip('-')

        # 1. Populate Highlights (Expertise) every run using deterministic IDs (upsert)
        ...
        defaults = [
            {
                ...
                'body': 'Expertise in machining operations, document control (FMEA, QCPC), and process improvement (Kaizen) for efficiency and quality.',
                'display_order': 1
            },
            {
                ...
                'body': 'Implementation of Machine Learning, RAG (Retrieval-Augmented Generation), Computer Vision, and predictive analytics.',
                'display_order': 2
            },
            {
                ...
                'body': 'Full-stack web application development using Python (Flask), HTML, CSS, and Database management for industrial solutions.',
                'display_order': 3
            },
            {
                ...
                'body': 'Advanced Excel automation and VBA macros for reporting, data processing, and dashboard creation.',
                'display_order': 4
            },
            {
                ...
                'body': 'Leading cross-functional teams, resource planning, and agile project execution.',
                'display_order': 5
            }
        ]
        for data in defaults:
            ...
            highlights_ref.document(doc_id).set(data, merge=True)

        # 2. Populate Quotes (Portfolio Projects) every run using deterministic IDs (upsert)
        quotes_ref = db.collection('quotes')
        projects = [
            {
                'project_name': 'Codex',
                'client_name': 'Internal / Corporate',
                'scope': 'Digital Signature Management System. Secure system for document authorization and digital signing workflows.',
                'status': 'Done'
            },
            {
                'project_name': 'PSD Analyst',
                'client_name': 'Quality Control Dept',
                'scope': 'Minitab Cloning. Statistical data analysis tool for quality control and process capability studies.',
                'status': 'Done'
            },
            {
                'project_name': 'Flux',
                'client_name': 'Legal Firm',
                'scope': 'Project Manager application. Comprehensive tool for managing legal cases, timelines, and documents.',
                'status': 'Done'
            },
            {
                'project_name': 'Kaizen System',
                'client_name': 'PT. Pakarti Riken Indonesia',
                'scope': 'Suggestion System. Digital platform for submitting, tracking, and rewarding continuous improvement ideas from employees.',
                'status': 'Done'
            },
            {
                'project_name': 'Monitorix',
                'client_name': 'Engineering Dept',
                'scope': 'Document Management System. Centralized repository for technical drawings, standards, and engineering documents.',
                'status': 'Done'
            },
            {
                'project_name': 'Control MI',
                'client_name': 'Quality Assurance',
                'scope': 'Control & Tracing Measuring Instrument. System for tracking calibration schedules and usage of measuring tools.',
                'status': 'Done'
            },
            {
                'project_name': 'Prime AI Assistant',
                'client_name': 'R&D',
                'scope': 'Chatbot Based RAG. Intelligent assistant using Retrieval-Augmented Generation to answer internal technical queries.',
                'status': 'In Progress'
            },
            {
                'project_name': 'Timesheet',
                'client_name': 'HR Department',
                'scope': 'Timesheet Online Form. Digital attendance and activity tracking system for workforce management.',
                'status': 'Done'
            }
        ]
        for p in projects:
            doc_id = _slug(p['project_name'])
            doc_ref = quotes_ref.document(doc_id)
            existing = doc_ref.get()
            token = generate_token()
            if existing.exists:
                existing_data = existing.to_dict() or {}
                token = existing_data.get('token', token)
            p.update({
                'amount': 0, # Hidden price
                'token': token,
                'created_at': firestore.SERVER_TIMESTAMP,
                'technical_approach': 'Standard implementation using Python/Flask and SQL/NoSQL databases.',
                'deliverables': 'Web Application, Source Code, User Manual.',
                'timeline': '3-6 Months',
                'tech_stack': 'Python, Flask, HTML/CSS, JS, SQL/Firestore',
                'team_structure': 'Full Stack Developer, Project Manager',
                'assumptions': 'Data provided by client.',
                'payment_terms': 'N/A'
            })
